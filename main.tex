...
% 目錄語言: \toggletrue = 中文目錄, \togglefalse = 英文目錄
\togglefalse{toc-use-cn}


% #### 下面是自己的資料 ####

% 碩士論文 → iamphd 設 false(博士才 true)
\togglefalse{iamphd}

% 論文名稱
\newcommand{\chineseTitle}{結合自適應提前終止與分層快取的\\磁碟式近似最近鄰搜尋}
\newcommand{\englishTitle}{Adaptive Early Termination and Hierarchical Caching for Disk-Based Approximate Nearest Neighbor Search}
% 論文名稱太長的話, 可以修改covers/inside_var.tex 的顯示語法, 詳情寫在那邊

% 自己的名字 (英文名字有兩種寫法, 一個是姓在後, 一個是放前面還有逗號. )
\newcommand{\studentCnName}{林冠霆}
\newcommand{\studentEnName}{Guan-Ting Lin} % 書名頁
\newcommand{\studentEnNameA}{Lin, Guan-Ting} % 封面用

% 教授的名字
\newcommand{\advisorCnName}{吳俊峯}
\newcommand{\advisorEnName}{Chun-Feng Wu}     % 書名頁
\newcommand{\advisorEnNamex}{Wu, Chun-Feng}   % 封面用
...
\newcommand{\NameofDepartmentInstituteCN}{多媒體工程研究所} % 系所名稱, 請看下方[說明1]
\newcommand{\NameofDepartmentInstituteEN}{Institute of Multimedia Engineering}
...
\input{Sections/3.Motivation} \newpage
\input{Sections/4.System} \newpage
